...
const FILE = 'file:///home/user/Planzy-saas/public/poc/kanvix-next-gen-v2.4.10.1.html' + NOW;
const SHOTS = '/home/user/Planzy-saas/recette-planning-bureau-v24101/';
...
console.log('\n[SYNC] Multi-onglets');
try {
  const HTTP = 'http://localhost:8241/kanvix-next-gen-v2.4.10.1.html' + NOW;
...
// ============================================================
// V2.4.10.1 — Fermeture des derniers chemins de mutation non sécurisés
// ============================================================

// KAN-TERM-01 / KAN-TERM-02 — Kanban : tâche terminée non draggable + drop refusé
console.log('\n[KAN-TERM] Kanban tâche terminée');
{
  const { ctx, p } = await newPage({ viewport: { width: 1440, height: 900 }, _tag: 'KANT' });
  await openPlan(p);
  await ev(p, () => { setPlanningView('kanban'); task('k-final').status = 'done'; save(); renderPage(); });
  await p.waitForTimeout(120);
  const r = await ev(p, (FAKE) => {
    const card = [...document.querySelectorAll('.kanban-card')].find((c) => (c.getAttribute('onclick') || '').includes("'k-final'"));
    const draggable = card ? card.getAttribute('draggable') : '(none)';
    const fe = eval('(' + FAKE + ')');
    const before = task('k-final').status;
    kanbanDragStart(fe, 'k-final'); kanbanDrop(fe, 'todo');
    return { draggable, moved: task('k-final').status !== before, status: task('k-final').status };
  }, FAKE);
  ok(r.draggable !== 'true', 'KAN-TERM-01 : carte Kanban terminée non draggable', 'KAN-TERM');
  ok(!r.moved && r.status === 'done', 'KAN-TERM-02 : kanbanDrop(done → todo) refusé (statut inchangé)', 'KAN-TERM');
  await ctx.close();
}

// STATUS-TERM-01 — setTaskStatus refuse de rouvrir une tâche terminée
console.log('\n[STATUS-TERM] setTaskStatus terminal');
{
  const { ctx, p } = await newPage({ viewport: { width: 1440, height: 900 }, _tag: 'STT' });
  await openPlan(p);
  const r = await ev(p, () => {
    task('k-final').status = 'done'; save();
    const u = app.undoStack.length;
    setTaskStatus('k-final', 'doing'); setTaskStatus('k-final', 'todo');
  ...
  const ok2 = await ev(p, () => { resetApp(); setDepth('pilot'); setTaskStatus('k-final', 'doing'); const a = task('k-final').status; setTaskStatus('k-final', 'done'); return a === 'doing' && task('k-final').status === 'done'; });
  ok(ok2, 'STATUS-TERM-01 : todo→doing→done toujours possibles (règles existantes préservées)', 'STATUS-TERM');
  await ctx.close();
}

// DONE-ACTIONS — fiche d'une tâche terminée : actions cohérentes
console.log('\n[DONE-ACTIONS] Actions fiche tâche terminée');
{
  const { ctx, p } = await newPage({ viewport: { width: 1440, height: 900 }, _tag: 'DA' });
  await openPlan(p);
  await ev(p, () => { task('k-windows').status = 'done'; task('k-windows').end = '2026-08-14T16:30'; save(); openTask('k-windows'); });
  await p.waitForTimeout(150);
  const r = await ev(p, () => {
    const txt = document.querySelector('#drawer').textContent || '';
    return { reprise: /Signaler une reprise/.test(txt), modifier: /Modifier la tâche/.test(txt), replan: /Replanifier/.test(txt), tester: /Tester un décalage/.test(txt), supprimer: /Supprimer/.test(txt) };
  });
  ok(r.reprise, 'DONE-ACTIONS : « Signaler une reprise » présent', 'DONE-ACTIONS');
  ok(!r.modifier && !r.replan && !r.tester && !r.supprimer, 'DONE-ACTIONS : Modifier / Replanifier / Tester / Supprimer absents', 'DONE-ACTIONS');
  await ctx.close();
}

// DONE-MOVE / DONE-DELETE — gardes défensives moveTask / deleteTask
console.log('\n[DONE-GUARDS] moveTask / deleteTask tâche terminée');
{
  const { ctx, p } = await newPage({ viewport: { width: 1440, height: 900 }, _tag: 'DG' });
  await openPlan(p);
  const mv = await ev(p, () => { task('k-windows').status = 'done'; save(); const s = task('k-windows').start, e = task('k-windows').end; moveTask('k-windows', 1); return task('k-windows').start === s && task('k-windows').end === e; });
  ok(mv, 'DONE-MOVE : moveTask(doneTask, 1) ne modifie pas les dates', 'DONE-GUARDS');
  const del = await ev(p, () => { resetApp(); setDepth('pilot'); task('k-final').status = 'done'; save(); const n = app.tasks.length; deleteTask('k-final'); confirmDeleteTask('k-final'); return !!task('k-final') && app.tasks.length === n; });
  ok(del, 'DONE-DELETE : deleteTask + confirmDeleteTask laissent la tâche terminée en place', 'DONE-GUARDS');
  await ctx.close();
}

// REPLAN — « Replanifier » via le moteur sûr (parité drag)
console.log('\n[REPLAN] Replanifier via moteur sûr');
{
  const { ctx, p } = await newPage({ viewport: { width: 1440, height: 900 }, _tag: 'RP' });
  await openPlan(p);
  const r = await ev(p, () => { const liningBefore = task('k-lining').start; moveTask('k-windows', 4); return { preview: document.querySelector('#modal').classList.contains('open'), txt: document.querySelector('#modalContent').textContent || '', liningUntouched: task('k-lining').start === liningBefore }; });
  ok(r.preview && /Impact/.test(r.txt), 'REPLAN : « Replanifier » ouvre l’aperçu d’impact (moteur sûr)', 'REPLAN');
  ok(r.liningUntouched, 'REPLAN : aucune tâche aval déplacée avant confirmation', 'REPLAN');
  ok(await ev(p, () => { applyTaskEditReflow(); return app.undoStack.length === 1; }), 'REPLAN : un seul snapshot Undo', 'REPLAN');
  ok(await ev(p, () => { resetApp(); setDepth('pilot'); go('planning'); task('k-lining').status = 'done'; save(); const b = task('k-lining').start; moveTask('k-windows', 4); if (document.querySelector('#modal').classList.contains('open')) applyTaskEditReflow(); return task('k-lining').start === b; }), 'REPLAN : successeur terminé jamais déplacé par « Replanifier »', 'REPLAN');
  await ctx.close();
}

// PARITE3 — Modifier == Drag == Replanifier (+1 jour)
console.log('\n[PARITE3] Parité Modifier / Drag / Replanifier');
{
  const { ctx, p } = await newPage({ viewport: { width: 1440, height: 900 }, _tag: 'P3' });
  await openPlan(p);
  const A = await ev(p, () => { resetApp(); setDepth('pilot'); go('planning'); openTaskEdit('k-windows', 'planning-gantt'); const f = document.querySelector('#taskEditForm'); f.querySelector('[name=start]').value = add(task('k-windows').start, DAY).slice(0, 16); f.querySelector('[name=end]').value = add(task('k-windows').end, DAY).slice(0, 16); markTaskEditDirty(); submitTaskEdit(); if (document.querySelector('#modal').classList.contains('open')) applyTaskEditReflow(); return JSON.stringify({ t: app.tasks.map((t) => ({ id: t.id, s: t.start, e: t.end })).sort((x, y) => x.id < y.id ? -1 : 1), u: app.undoStack.length }); });
  const B = await ev(p, () => { resetApp(); setDepth('pilot'); go('planning'); requestGanttTaskMove('k-windows', add(task('k-windows').start, DAY)); if (document.querySelector('#modal').classList.contains('open')) applyTaskEditReflow(); return JSON.stringify({ t: app.tasks.map((t) => ({ id: t.id, s: t.start, e: t.end })).sort((x, y) => x.id < y.id ? -1 : 1), u: app.undoStack.length }); });
  const C = await ev(p, () => { resetApp(); setDepth('pilot'); go('planning'); moveTask('k-windows', 1); if (document.querySelector('#modal').classList.contains('open')) applyTaskEditReflow(); return JSON.stringify({ t: app.tasks.map((t) => ({ id: t.id, s: t.start, e: t.end })).sort((x, y) => x.id < y.id ? -1 : 1), u: app.undoStack.length }); });
  ok(A === B && B === C, 'PARITE3 : état métier final identique pour Modifier / Drag / Replanifier (+1 jour)', 'PARITE3');
  await ctx.close();
}

// SIM-DONE — scénario avec tâche terminée en aval
console.log('\n[SIM-DONE] Simulation tâche terminée en aval');
{
  const { ctx, p } = await newPage({ viewport: { width: 1440, height: 900 }, _tag: 'SD' });
  await openPlan(p);
  const r = await ev(p, () => { task('k-lining').status = 'done'; save(); const a = scenarioOptions('windows-delay').find((x) => x.id === 'a'); const targets = a.changes.map((c) => c.taskId); app.simulation = a; applySimulation(); return { targetsLining: targets.includes('k-lining'), liningStart: task('k-lining').start }; });
  ok(!r.targetsLining, 'SIM-DONE : tâche terminée en aval absente des changements du scénario', 'SIM-DONE');
  ok(r.liningStart === '2026-08-17T08:00', 'SIM-DONE : dates de la tâche terminée inchangées après application', 'SIM-DONE');
  await ctx.close();
}

// SIM-DOING — scénario avec tâche en cours en aval
console.log('\n[SIM-DOING] Simulation tâche en cours en aval');
{
  const { ctx, p } = await newPage({ viewport: { width: 1440, height: 900 }, _tag: 'SG' });
  await openPlan(p);
  const r = await ev(p, () => { task('k-lining').status = 'doing'; save(); const a = scenarioOptions('windows-delay').find((x) => x.id === 'a'); const rEval = evaluateScenario(a); const before = task('k-lining').start; app.simulation = a; applySimulation(); return { reqArb: rEval.requiresArbitration, targetsLining: a.changes.some((c) => c.taskId === 'k-lining'), moved: task('k-lining').start !== before, modal: document.querySelector('#modal').classList.contains('open') }; });
  ok(r.reqArb && !r.targetsLining, 'SIM-DOING : tâche en cours non déplacée + scénario marqué arbitrage nécessaire', 'SIM-DOING');
  ok(!r.moved && r.modal, 'SIM-DOING : aucune application silencieuse (message d’arbitrage)', 'SIM-DOING');
  await ctx.close();
}

// APPLY-DEF — garde défensive applySimulation (changement done injecté)
console.log('\n[APPLY-DEF] Garde défensive applySimulation');
{
  const { ctx, p } = await newPage({ viewport: { width: 1440, height: 900 }, _tag: 'AD' });
  await openPlan(p);
  const r = await ev(p, () => { task('k-lining').status = 'done'; save(); app.simulation = { id: 'x', projectId: 'keravel', changes: [ { taskId: 'k-windows', start: add(task('k-windows').start, 2 * DAY), end: add(task('k-windows').end, 2 * DAY) }, { taskId: 'k-lining', start: '2026-08-25T08:00', end: '2026-08-26T16:00' } ] }; const before = task('k-lining').start; applySimulation(); return { moved: task('k-lining').start !== before, refused: /Application impossible/.test(document.querySelector('#modalContent')?.textContent || '') }; });
  ok(!r.moved && r.refused, 'APPLY-DEF : application refusée si un changement cible une tâche terminée (done inchangée)', 'APPLY-DEF');
  await ctx.close();
}

// RESOLVE — scénarios Résoudre sans déplacement interdit
console.log('\n[RESOLVE] Scénarios Résoudre sûrs');
{
  const { ctx, p } = await newPage({ viewport: { width: 1440, height: 900 }, _tag: 'RS' });
  await openPlan(p);
  const r = await ev(p, () => { task('k-lining').status = 'done'; task('k-paint').status = 'doing'; save(); const opts = scenarioOptions('windows-delay'); return { anyDoneMove: opts.some((s) => evaluateScenario(s).statusViolations.done.length > 0) }; });
  ok(!r.anyDoneMove, 'RESOLVE : aucun scénario proposé ne déplace une tâche terminée', 'RESOLVE');
  await ctx.close();
}
