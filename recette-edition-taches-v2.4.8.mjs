...
console.log('\n[EDIT-07] Statut manuel');
{
  ...
  const r = await ev(p, () => ({
    ...
    hist: app.history[0]?.text,
    /* V2.8.5.2 — RE-BASELINE. Un démarrage réel confirmé replace aussi les
       dates (V2.8.5.1 §7) : l'éditeur produit donc, EN PLUS de l'entrée de
       transition, son entrée de synthèse « … modifiée. » qui se retrouve en
       tête. L'entrée de transition existe toujours, avec le MÊME texte — c'est
       elle qu'on vérifie, sans supposer qu'elle est la plus récente. */
    histTransition: app.history.map((h) => h.text).find((t) => /statut modifié manuellement/.test(t || '')),
  }));
  ...
  ok(/statut modifié manuellement\s*:\s*À faire\s*→\s*En cours/.test(r.histTransition || ''), 'EDIT-07 : historique "statut modifié manuellement : À faire → En cours"', 'EDIT-07');
  await ctx.close();
}
