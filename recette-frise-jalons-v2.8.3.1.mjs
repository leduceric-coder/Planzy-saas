// ============================================================
// KANVIX — Recette « Frise de jalons » (V2.8.3.1)
//
//   Copie CONFORME de recette-frise-jalons-v2.8.3.mjs. DEUX assertions et
//   deux seules ont été remplacées, parce qu'elles encodaient la règle de
//   statut que V2.8.3.1 corrige — FRISE-14 et FRISE-17. Tout le reste est
//   repris au caractère près : la frise, elle, n'a pas bougé.
//
...
console.log('\n[FRISE-CARTES] Statut métier et contenu des cartes');
{
  ...
  await openJalons(p);
  await seedRich(p);
  // V2.8.3.1 — deux alertes LÉGITIMES sous les nouvelles règles, pour que ce
  // bloc continue d'observer l'éventail complet des statuts : un jalon FINAL
  // dont le chantier dérive, et un jalon LIÉ à une tâche non terminée dont la
  // date est passée. V2.8.3 obtenait ses alertes de la règle erronée.
  await ev(p, () => {
    const ids = operationProjectIds(window.OP);
    app.tasks.push({ id: 'tk-alerte', projectId: ids[0], name: 'Reprise', resourceId: null, status: 'todo',
      start: shiftDateStr(dayKey(TODAY), -4) + 'T08:00', end: shiftDateStr(dayKey(TODAY), -3) + 'T17:00' });
    app.milestones.push({ id: 'm-final', projectId: ids[0], name: 'Livraison', isFinal: true, taskId: null,
      date: shiftDateStr(dayKey(TODAY), 3) + 'T17:00' });
    app.milestones.push({ id: 'm-lie', projectId: ids[0], name: 'Reprise attendue', isFinal: false, taskId: 'tk-alerte',
      date: shiftDateStr(dayKey(TODAY), -2) + 'T17:00' });
    save(); renderPage();
  });
  ...
  // FRISE-14 — REMPLACÉE EN V2.8.3.1. L'attendu de V2.8.3 encodait la règle
  // même que ce lot corrige : il appliquait la fin de chantier à TOUS les
  // jalons et déduisait « Atteint » de l'absence d'intervention ouverte. Il est
  // recalculé ici avec le nouvel arbre de décision — tâche associée, puis jalon
  // final, puis jalon intermédiaire — toujours à partir des seuls moteurs
  // existants, sans appeler milestoneStatus().
  const st = await ev(p, () => {
    const list = visibleOperationMilestones(window.OP), now = getDemoNow();
    return list.map((m) => {
      const p0 = project(m.projectId);
      const past = !!(m.date && date(m.date) < now);
      const t = m.taskId ? task(m.taskId) || null : null;
      let attendu;
      if (p0 && !isProjectActive(p0)) attendu = isProjectArchived(p0) ? 'Chantier archivé' : 'Chantier clôturé';
      else if (t) attendu = t.status === 'done' ? 'Atteint'
        : past ? 'Dépassé'
          : m.date && t.end && date(t.end) > date(m.date) ? 'Menacé' : 'À venir';
      else if (m.isFinal === true) {
        const end = calculateProjectEnd(m.projectId);
        attendu = past ? 'Dépassé' : (m.date && end && date(end) > date(m.date)) ? 'Menacé' : 'À venir';
      } else attendu = past ? 'Date passée' : 'À venir';
  ...
  ok(st.every((x) => x.moteur === x.rendu) && domStatus.join('|') === st.map((x) => x.moteur).join('|')
    ...
    'FRISE-14 : le statut de chaque jalon est recalculable depuis les seuls moteurs existants (isProjectActive, calculateProjectEnd, task) selon l’arbre tâche → final → intermédiaire — au moins quatre statuts distincts observés, aucun inventé', 'FRISE-14');
  ...
  // FRISE-17 — REMPLACÉE EN V2.8.3.1. L'invariant testé — bulle et liseré de
  // même couleur, l'alerte primant sur la teinte d'identité — est INCHANGÉ et
  // reste vérifié. Seule sa mise en situation devait bouger : le scénario de
  // V2.8.3 produisait ses alertes grâce à la règle erronée (des jalons
  // intermédiaires déclarés « Menacé » par la fin de chantier). On provoque
  // désormais deux alertes LÉGITIMES — un jalon final en dérive et un jalon
  // lié à une tâche non terminée dont la date est passée.
  ...
  ok(col.every((c) => c.bub === c.card)
    && col.some((c) => c.st === 'st-risk') && col.some((c) => c.st === 'st-late'),
    'FRISE-17 : bulle et liseré de carte portent toujours la même couleur — et une alerte LÉGITIME (jalon final en dérive, jalon lié à une tâche en retard) prend le pas sur la teinte d’identité du chantier', 'FRISE-17');
...
console.log('\n[FREEZE-283] Byte-identité et périmètre');
{
  ...
  const frozen = [
    ...
    'milestoneOpenTasks', 'visibleOperationMilestones', 'operationMilestoneBar',
  ...
  ok(changed.length === 3 && added.length === 1,
    'FREEZE-283 : par rapport à V2.8.3, seules milestoneStatus(), operationMilestoneStats() et operationMilestonesTab() ont bougé, et un helper pur a été ajouté', 'FREEZE-283');
