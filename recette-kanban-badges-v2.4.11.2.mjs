// ============================================================
// KANVIX — Recette badges Kanban « POINT OUVERT » vs « SOUS IMPACT » (V2.4.11.2)
//   Usage : node recette-kanban-badges-v2.4.11.2.mjs
// ============================================================
import pw from '/home/user/Planzy-saas/node_modules/playwright-core/index.js';
import fs from 'fs';
const { chromium } = pw;
const NOW = '?now=2026-08-13T09:00:00';
const FILE = 'file:///home/user/Planzy-saas/public/poc/kanvix-next-gen-v2.4.11.2.html' + NOW;
const SHOTS = '/home/user/Planzy-saas/recette-kanban-badges-v2.4.11.2/';
...
// ---- BADGE-01 : incident direct → POINT OUVERT ----
console.log('\n[BADGE-01] Incident direct → POINT OUVERT');
{
  ...
  note('BADGE-01', c);
  ok(c['Tache A source'] && c['Tache A source'].attention, 'BADGE-01 : A (incident ouvert direct) affiche POINT OUVERT', 'BADGE-01');
  ok(c['Tache A source'] && !c['Tache A source'].impacted, 'BADGE-01 : A n’affiche PAS SOUS IMPACT en plus', 'BADGE-01');
  ok(c['Tache A source'] && c['Tache A source'].labels.some((l) => /POINT OUVERT/i.test(l)), 'BADGE-01 : libellé rendu « POINT OUVERT »', 'BADGE-01');
  await ctx.close();
}
...
console.log('\n[BADGE-02] Chaîne A→B→C, incident sur A');
{
  ...
  await setupScenario(p);
  const c = await readCards(p);
  ok(c['Tache A source'].attention && !c['Tache A source'].impacted, 'BADGE-02 : A → POINT OUVERT', 'BADGE-02');
  ok(c['Tache B aval'].impacted && !c['Tache B aval'].attention, 'BADGE-02 : B → SOUS IMPACT (et pas POINT OUVERT)', 'BADGE-02');
  ok(c['Tache C aval'].impacted && !c['Tache C aval'].attention, 'BADGE-02 : C → SOUS IMPACT (et pas POINT OUVERT)', 'BADGE-02');
...
console.log('\n[BADGE-03] B → En cours : reste SOUS IMPACT');
{
  ...
  ok(!after['Tache B aval'].attention, 'BADGE-03 : B ne devient PAS POINT OUVERT en changeant de statut', 'BADGE-03');
  ok(after['Tache A source'].attention, 'BADGE-03 : A reste POINT OUVERT', 'BADGE-03');
...
console.log('\n[BADGE-04] Incident de A résolu → plus aucun badge');
{
  ...
  const c = await readCards(p);
  note('BADGE-04', c);
  ok(!c['Tache A source'].attention, 'BADGE-04 : A ne porte plus POINT OUVERT', 'BADGE-04');
...
console.log('\n[BADGE-05] B a son propre incident → POINT OUVERT prioritaire');
{
  ...
  const c = await readCards(p);
  note('BADGE-05', c);
  ok(c['Tache A source'].attention, 'BADGE-05 : A → POINT OUVERT', 'BADGE-05');
  ok(c['Tache B aval'].attention, 'BADGE-05 : B → POINT OUVERT (incident propre)', 'BADGE-05');
...
console.log('\n[BADGE-08] Filtre « Tous » → « Résidence Keravel »');
{
  ...
  ok(kWindows && kWindows.attention && !kWindows.impacted, 'BADGE-08 : « Pose des 6 fenêtres » (incident direct) → POINT OUVERT', 'BADGE-08');
...
console.log('\n[BADGE-09] Tâche neutre → aucun badge d’attention');
{
  ...
  ok(z && !z.attention, 'BADGE-09 : aucune mention POINT OUVERT', 'BADGE-09');
...
console.log('\n[BADGE-10] REPRISE cumulable avec le badge d’attention');
{
  ...
  ok(c['Tache A source'].rework && c['Tache A source'].attention, 'BADGE-10 : A → REPRISE + POINT OUVERT', 'BADGE-10');
  ok(c['Tache B aval'].rework && c['Tache B aval'].impacted, 'BADGE-10 : B → REPRISE + SOUS IMPACT', 'BADGE-10');
  ok(!c['Tache B aval'].attention, 'BADGE-10 : B ne bascule pas en POINT OUVERT à cause de la reprise', 'BADGE-10');
...
console.log('\n[BADGE-11] Dark mode');
{
  ...
  ok(cols.impacted.fg !== cols.attention.fg, 'BADGE-11 : SOUS IMPACT ≠ POINT OUVERT (couleur de texte)', 'BADGE-11');
  ok(cols.impacted.bg !== cols.attention.bg, 'BADGE-11 : SOUS IMPACT ≠ POINT OUVERT (fond)', 'BADGE-11');
...
console.log('\n[BADGE-12] Mode clair — différenciation');
{
  ...
  ok(cols.attention.txt === 'POINT OUVERT', 'BADGE-12 : libellé exact « POINT OUVERT » conservé', 'BADGE-12');
...
console.log('\n[BADGE-19] Tableau électrique démarré → POINT OUVERT');
{
  ...
  ok(before['Tableau électrique'].attention, 'BADGE-19 : À faire → POINT OUVERT', 'BADGE-19');
  // Passage À faire → En cours via le moteur de statut existant (non modifié).
  await ev(p, () => { setTaskStatus('k-electric', 'doing', 'test'); /* V2.8.5.2 — le passage « En cours » demande désormais confirmation (§7) : on confirme, comme l'utilisateur. */ /* V2.12.0 — RE-POINTAGE. Le passage « En cours » traverse désormais aussi la garde des CONDITIONS DE DÉMARRAGE (§12) : on confirme, comme l'utilisateur, exactement comme on confirmait déjà le calendrier depuis V2.8.5.2. L'exigence testée est inchangée. */ if (document.querySelector('#modal.open [data-prq-confirm]')) confirmPrerequisiteStart(); if (document.querySelector('#modal.open [data-calendar-confirm]')) runCalendarConfirm(); renderPage(); });
  await p.waitForTimeout(220);
  const after = await readCards(p);
  ...
  ok(after['Tableau électrique'].attention, 'BADGE-19 : En cours + POINT OUVERT (le badge survit au démarrage)', 'BADGE-19');
  ok(after['Tableau électrique'].labels.includes('POINT OUVERT'), 'BADGE-19 : libellé rendu « POINT OUVERT »', 'BADGE-19');
  ok(!after['Tableau électrique'].labels.some((l) => /À TRAITER/i.test(l)), 'BADGE-19 : INTERDIT — aucun « À TRAITER » sur la carte démarrée', 'BADGE-19');
...
console.log('\n[BADGE-20] Conséquence pure « À faire → En cours »');
{
  ...
  await ev(p, () => { setTaskStatus('k-paint', 'doing', 'test'); /* V2.8.5.2 — le passage « En cours » demande désormais confirmation (§7) : on confirme, comme l'utilisateur. */ /* V2.12.0 — RE-POINTAGE. Le passage « En cours » traverse désormais aussi la garde des CONDITIONS DE DÉMARRAGE (§12) : on confirme, comme l'utilisateur, exactement comme on confirmait déjà le calendrier depuis V2.8.5.2. L'exigence testée est inchangée. */ if (document.querySelector('#modal.open [data-prq-confirm]')) confirmPrerequisiteStart(); if (document.querySelector('#modal.open [data-calendar-confirm]')) runCalendarConfirm(); renderPage(); });
  await p.waitForTimeout(220);
  const after = await readCards(p);
  ...
  ok(!after['Peinture étage 1'].attention, 'BADGE-20 : elle ne bascule pas en POINT OUVERT', 'BADGE-20');
  await p.screenshot({ path: SHOTS + '13-consequence-en-cours.png', fullPage: true });
  await ctx.close();
}

// ---- BADGE-21 : plus aucun « À TRAITER » dans le Kanban, conservé ailleurs (§13/§7) ----
console.log('\n[BADGE-21] « À traiter » retiré du Kanban, conservé ailleurs');
{
  ...
  ok(kanban.aTraiter === 0, 'BADGE-21 : 0 badge « À TRAITER » rendu dans le Kanban', 'BADGE-21');
  ok(!/À traiter/i.test(kanban.cardsHTML), 'BADGE-21 : la chaîne « À traiter » est absente du HTML du tableau Kanban', 'BADGE-21');
  ok(kanban.pointOuvert > 0, 'BADGE-21 : des badges « POINT OUVERT » sont bien rendus', 'BADGE-21');
