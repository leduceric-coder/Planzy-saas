...
console.log('\n[PLAN-F4] Drag Gantt vs édition universelle (CRITIQUE)');
{
  ...
  const undoDrag = await ev(p, (FAKE) => {
    ...
    /* V2.8.5.2 — RE-BASELINE : depuis V2.8.5.1 (§5), déposer une tâche sur un
       samedi, un dimanche ou un férié demande confirmation AVANT toute écriture.
       Le geste la comporte donc ; l'exigence testée — « Undo restaure
       exactement les dates » — est INCHANGÉE. */
