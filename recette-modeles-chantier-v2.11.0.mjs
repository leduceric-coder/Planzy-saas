...
console.log('\n[TPL-CAPTURE] Enregistrer une trame');
{
  ...
  ok(champs.join(',') === 'deps,durationDays,endTime,id,lotId,name,offsetDays,phase,startTime,structureNodeId,suggestedResourceId',
    'TPL-15 : une intervention de modèle porte EXACTEMENT onze champs — identité, rattachement, lot, phase, jours relatifs, heures, dépendances et intervenant SUGGÉRÉ', 'TPL-15');
...
console.log('\n[TPL-RESP] Largeurs, thème, clavier');
{
  ...
  note('TPL-55', form);
  ok(form.ouvert && form.titre === 'ENREGISTRER COMME MODÈLE' && form.champs.join('|') === 'name|description'
    && form.valeur === 'Bâtiment A' && /niveau/.test(form.resume),
    'TPL-55 : la side-window « Enregistrer comme modèle » s’ouvre préremplie au nom du niveau et annonce le périmètre MESURÉ qu’elle va capturer', 'TPL-55');
  await ctx.close();
}
