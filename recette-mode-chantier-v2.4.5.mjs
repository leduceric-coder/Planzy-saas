...
const FILE = 'file:///home/user/Planzy-saas/public/poc/kanvix-next-gen-v2.4.5.html' + NOW;
const HTTP = 'http://localhost:8241/kanvix-next-gen-v2.4.5.html' + NOW;
const SHOTS = '/home/user/Planzy-saas/recette-mode-chantier/';
...
{
  const html = fs.readFileSync('/home/user/Planzy-saas/public/poc/kanvix-next-gen-v2.4.5.html', 'utf8');
...
// ============================================================
// FINDING-1 — "Voir dans le planning" du popup d'impact reprise
// (candidat MAJOR : utilise focusPlanningTask/app.ui.page='planning'
//  au lieu de openFieldPlanning → n'atteint pas le planning mobile)
// ============================================================
console.log('\n[FINDING-1] "Voir dans le planning" du popup impact reprise');
{
  const { ctx, p } = await newPage({ viewport: { width: 390, height: 844 }, _tag: 'FINDING-1' });
  await ev(p, () => {
    resetApp(); setDepth('pilot');
    task('k-windows').status = 'done';
    setRole('driver'); enterFieldMode(); selectFieldProject('keravel');
    createRework('k-windows', { comment: 'Test finding planning.', dataUrl: null, planDate: '2026-08-17', duration: 'half' });
  });
  await p.waitForTimeout(200);
  const btnExists = await ev(p, () => !!([...document.querySelectorAll('#modalContent .btn')].find((x) => /Voir dans le planning/.test(x.textContent))));
  note('FINDING-1', 'bouton "Voir dans le planning" présent dans le popup impact', btnExists);
  if (btnExists) {
    await ev(p, () => [...document.querySelectorAll('#modalContent .btn')].find((x) => /Voir dans le planning/.test(x.textContent)).click());
    await p.waitForTimeout(250);
    const after = await ev(p, () => ({
      ...
      page: app.ui.page,
      fieldView: app.ui.fieldView,
      planningVisible: !!document.querySelector('.field-plan-focus') || !!document.querySelector('.field-range-toggle'),
      chantierHomeVisible: !!document.querySelector('.field-project-head'),
    }));
    note('FINDING-1', 'état après clic', after);
    // Observation neutre (pas d'assertion — §54 : ne pas coder un bug comme
    // résultat attendu, mais ne pas non plus asserter le bug comme correct).
    // Le popup tâche ouvre bien le planning mobile (openFieldPlanning, cf.
    // PM-F3) ; ici le même libellé s'appuie sur focusPlanningTask (Bureau) →
    // app.ui.page='planning' est ignoré par renderField, fieldView reste
    // "site", donc l'utilisateur revient à l'accueil chantier au lieu du
    // planning. Aucune impasse, aucune sortie du Mode Chantier — voir
    // finding MC-01 dans le rapport.
    const reproduced = after.field && after.fieldView === 'site' && !after.planningVisible && after.chantierHomeVisible;
    note('FINDING-1', 'MC-01 reproduit (bouton n\'atteint pas le planning mobile, pas d\'impasse)', reproduced);
  }
  await ctx.close();
}
